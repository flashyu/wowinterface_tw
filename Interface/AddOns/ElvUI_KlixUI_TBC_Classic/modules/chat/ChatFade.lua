local KUI, T, E, _, V, P, G = unpack(select(2, ...))
...
function KC:HideChatFade()

	E:UIFrameFadeOut(self.fadeParent, 0.2, self.fadeParent:GetAlpha(), E.db.KlixUI.chat.chatFade.minAlpha)
end

function KC:ShowChatFade()
	self.timeout = 0
...
function KC:OnUpdate(elapsed)
	...
	if not InCombatLockdown() and not self.editboxforced then
		...
		if self.timeout > E.db.KlixUI.chat.chatFade.timeout then
			self:HideChatFade()
		end
	end
end

function KC:LoadChatFade()
	self.fadeParent = CreateFrame("Frame", nil, _G.UIParent)
	self.fadeParent:SetAlpha(1 - E.db.KlixUI.chat.chatFade.minAlpha)
	self.fadeParent:SetFrameStrata("BACKGROUND")
	self.timeout = 0
	self:Configure_ChatFade()
end

function KC:OnEditFocusGained()
	self.editboxforced = true
	self:ShowChatFade()
end

function KC:OnEditFocusLost()
	self.editboxforced = false
end

function KC:Configure_ChatFade()
	if E.db.KlixUI.chat.chatFade.enable then
