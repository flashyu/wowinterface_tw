-------------------------------------------------------------------------------
-- Based on: Leatrix Maps - Leatrix
-------------------------------------------------------------------------------
local KUI, T, E, L, V, P, G = unpack(select(2, ...))
local KWM = KUI:NewModule('KuiWorldMap', 'AceHook-3.0');

function KWM:UpdateMapScale()
	if self.isMaximized and self:GetScale() ~= 1 then
		self:SetScale(1)
	elseif not self.isMaximized and self:GetScale() ~= KWM.db.scale or 0.7 then
		self:SetScale(KWM.db.scale or 0.7)
	end
end

function KWM:UpdateMapAnchor()
	KWM.UpdateMapScale(self)
...
local function RefMap(self, fullUpdate)
	...
	local artID = T.C_Map_GetMapArtID(mapID); if not artID or not LeaMapsData[artID] then return end
	local LeaMapsZone = LeaMapsData[artID]

	-- Store already explored tiles in a table so they can be ignored
	local TileExists = {}
	local exploredMapTextures = T.C_MapExplorationInfo_GetExploredMapTextures(mapID)
	...
	local layers = T.C_Map_GetMapArtLayers(mapID)
...
KUI:RegisterModule(KWM:GetName())