...
local function jiantoufangxiang(self,sorted)
	if sorted then
		if tocversion<20000 then
			self.paixu:SetRotation(math.rad(-180), 0.29, 0.6)
		elseif tocversion<40000 then
			self.paixu:SetRotation(math.rad(-180),{x=0.29,y=0.6})
		end
	else
		if tocversion<20000 then
			self.paixu:SetRotation(0, 0.29, 0.6)
		elseif tocversion<40000 then
			self.paixu:SetRotation(0,{x=0.29,y=0.6})
		end
	end
	self.paixu:Show()
end
function CommonFun.AHPlus_Vanilla()
	...
	AuctionFrameBrowse.exact =PIGCheckbutton(AuctionFrameBrowse,{"TOPLEFT",AuctionFrameBrowse,"TOPLEFT",530,-13},{"精确查找","选中后查找结果将精确匹配关键字"})
	...
	local function SetBlizzardUI(tmV)
		...
		if BrowsePriceOptionsButtonFrame then--设置已单件展示
			BrowsePriceOptionsButtonFrame:ClearAllPoints();
			BrowsePriceOptionsButtonFrame:SetPoint("TOPRIGHT",AuctionFrameBrowse,"TOPRIGHT",74,-76);
		end
	...
	if tocversion<20000 then
		listF.fengeline:SetRotation(math.rad(-90), 0, 0)
	elseif tocversion<40000 then
	...
	listF.tishi = PIGFontString(listF,{"CENTER", listF, "CENTER", 0,100},"选择搜索条件,然后按下“搜索”")
	---
	...
	local function chuangjianjinbiF(fujiF,Point,width,hang_Height)
		...
		frame.TV = PIGFontString(frame,{"RIGHT", frame.T, "LEFT", 0,0},nil,nil,13)
		...
		frame.YV = PIGFontString(frame,{"RIGHT", frame.Y, "LEFT", 0,0},nil,nil,13)
		...
		frame.GV = PIGFontString(frame,{"RIGHT", frame.G, "LEFT", 0,0},nil,nil,13)
	...
	for i = 1, hang_NUM do
		...
		listFitem.itemlink.t = PIGFontString(listFitem.itemlink,nil,nil,nil,13)
		listFitem.itemlink.t:SetAllPoints(listFitem.itemlink)
		listFitem.itemlink.t:SetJustifyH("LEFT");
		---
		listFitem.count = PIGFontString(listFitem,{"LEFT", listFitem.itemlink, "RIGHT", 0,0},nil,nil,13)
		listFitem.count:SetWidth(xulieID_www[2]);
		listFitem.count:SetJustifyH("LEFT");
		--
		listFitem.TimeLeft = PIGFontString(listFitem,{"LEFT", listFitem.count, "RIGHT", 0,0},nil,nil,13)
		...
		--
		listFitem.zhangdie = PIGFontString(listFitem,{"LEFT", listFitem.yikoudanjia, "RIGHT", 0,0},nil,nil,13)
		listFitem.zhangdie:SetWidth(xulieID_www[7]);
		listFitem.zhangdie:SetJustifyH("LEFT");
		----
		listFitem.chushouzhe = PIGFontString(listFitem,{"LEFT", listFitem.zhangdie, "RIGHT", 20,0},nil,nil,13)
	...
	HCUI.jindu.edg.t = PIGFontString(HCUI.jindu.edg,{"CENTER",HCUI.jindu.edg,"CENTER",0,0},"正在获取数据...",nil,13)
	----
	...
	---------------------
	for i = 1, 33 do
		local huizhangG = PIGFontString(BrowseWowTokenResults,nil,nil,nil,13,"huizhangG_"..i)
		if i==1 then
	...
	AuctionFrame:HookScript("OnShow",function(self)
		...
		piglist_biaoti_4.paixu:Hide()
		piglist_biaoti_5.paixu:Hide()
		piglist_biaoti_6.paixu:Show()
		if tocversion<20000 then
			piglist_biaoti_6.paixu:SetRotation(0, 0.5, 0.5)
		elseif tocversion<40000 then
			piglist_biaoti_6.paixu:SetRotation(0,{x=0,y=0})
		end
		SetSelectedAuctionItem("list", 0);
	end)
	...
	coll.list.title = PIGFontString(coll.list,{"TOP", coll.list, "TOP", -2, -6},"收藏夹")
	...
	for i = 1, collhang_NUM do
		...
		colllistitem.link = PIGFontString(colllistitem,{"LEFT", colllistitem.icon, "RIGHT", 0,0},nil,nil,13)
	...
	SellListF.tishi = PIGFontString(SellListF,{"CENTER", SellListF, "CENTER", 0,0},"没有此物品在售卖，无参考价！")
	--
	...
	for i = 1, spellhangnum do
		...
		---
		listFitem.count = PIGFontString(listFitem,{"LEFT", listFitem.yikoudanjia, "RIGHT", 0,0},nil,nil,13)
		...
		--
		listFitem.chushouzhe = PIGFontString(listFitem,{"LEFT", listFitem.TimeLeft, "RIGHT", 2,0},nil,nil,13)
	...
	--压价按钮
	AuctionFrameAuctions.yajingbiao =PIGCheckbutton(AuctionFrameAuctions,{"TOPLEFT",AuctionFrameAuctions,"TOPLEFT",24,-284},{"压竞标价","选中后压一口价同时压竞标价"})
	...
	AuctionFrameAuctions.Showcankao:SetSize(70,20);
	AuctionFrameAuctions.Showcankao:SetPoint("LEFT",AuctionFrameAuctions.yajingbiao,"RIGHT",80,0);
	AuctionFrameAuctions.Showcankao:SetText("参考价");
