...
function UnitFramefun.Zishen()
	...
	if PIGA["UnitFrame"]["PlayerFrame"]["HPFF"] and not PlayerFrame.ziji then
		...
		PlayerFrame.zhixingcishu=0
		local function zhixingyidongP()
			local offset1 = PlayerFrame.ziji:GetRight()
			local offset2 = TargetFrame:GetLeft()
			local offset3 = PlayerFrame:GetBottom()
			local offset4 = TargetFrame:GetBottom()
			if offset1 and offset2 and offset3 and offset4 and PlayerFrame.zhixingcishu<20 then
				if offset1>offset2 and offset3==offset4 then
					TargetFrame:ClearAllPoints();
					TargetFrame:SetPoint("TOPLEFT",UIParent,"TOPLEFT",350,-4);
					TargetFrame:SetUserPlaced(true);
				end
			else
				PlayerFrame.zhixingcishu=PlayerFrame.zhixingcishu+1
