...
local function FindItem( loc_id, cl, cb, bp, cs, id, ct )
	...
	if loc_id == ArkInventory.Const.Location.Bank and cb == ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK and ArkInventory.db.option.transfer.topup then
...
local function FindNormalItem( loc_id, cl, cb, bp, cs )
	...
	for bag_pos, bag_id in ipairs( ArkInventory.Global.Location[loc_id].Bags ) do
		...
		if not me.player.data.option[loc_id].bag[bag_id].transfer.ignore then
			...
			if bt == 0 and bag_id ~= ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK then
	...
	for bag_pos, bag_id in ipairs( ArkInventory.Global.Location[loc_id].Bags ) do
		...
		if not me.player.data.option[loc_id].bag[bag_id].transfer.ignore then
			...
			if ArkInventory.db.option.transfer.priority then
				...
				if bag_id ~= ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK and ( bt == 0 or bt == ct ) then
							...
							if h then
								...
								if info.equiploc ~= "INVTYPE_BAG" then
									...
									if check_item then
										...
										if ArkInventory.ClientCheck( ArkInventory.Const.ENUM.EXPANSION.CLASSIC ) then -- FIX ME
...
local function FindCraftingItem( loc_id, cl, cb, bp, cs )
	...
	for bag_pos, bag_id in ipairs( ArkInventory.Global.Location[loc_id].Bags ) do
		...
		if not me.player.data.option[loc_id].bag[bag_id].transfer.ignore then
			...
			if ArkInventory.db.option.transfer.priority then
				-- priority is reagent bank
				if bag_id ~= ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK and ( bt == 0 or cb == ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK ) then
					-- do not steal from the reagent bank
					-- do not steal from profession bags, unless its for the reagent bank
					pri_ok = true
				end
...
local function Consolidate( loc_id )
	...
	for bag_pos = #ArkInventory.Global.Location[loc_id].Bags, 1, -1 do
		...
		if not me.player.data.option[loc_id].bag[bag_id].transfer.ignore then
			...
			if count > 0 and ( bag_id == ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK or bt ~= 0 ) then
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if ArkInventory.db.option.transfer.deposit and ArkInventory.CrossClient.IsReagentBankUnlocked( ) then
			...
			local bag_id = ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK
...
local function Compact( loc_id )
	...
	for bag_pos = #ArkInventory.Global.Location[loc_id].Bags, 1, -1 do
		...
		if not me.player.data.option[loc_id].bag[bag_id].transfer.ignore then
			...
			if count > 0 and bt == 0 and bag_id ~= ArkInventory.Const.ENUM.BAG.INDEX.REAGENTBANK then
...
--[[
	...
	if loc_id == ArkInventory.Const.Location.Bank then
		...
		if ArkInventory.Global.Mode.Bank then
			...
			if ArkInventory.ClientCheck( ArkInventory.Const.ENUM.EXPANSION.CLASSIC ) and ArkInventory.db.option.transfer.blizzard then -- FIX ME
