...
function NS.ShowPlayerTooltip(GUID)
  ...
  if GUID and NS.PlayerActiveCache[GUID] then
    ...
    local level = FormatLevelString(estimated, NS.PlayerDB[fullName].L)
    ...
    if NS.KosList[fullName] then
      titleLeft = titleLeft .. "|TInterface/Addons/weizPVP/Media/Icons/kos.tga:0|t |cFFFF0040>|r" .. WrapTextInColorCode(formattedName .. "|cFFFF0040<|r", select(4, GetClassColor(class)))
    else
