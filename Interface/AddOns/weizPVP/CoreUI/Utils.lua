...
function NS.SetCoreFramePosition()
  ...
  weizPVP_CoreFrame:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", NS.Options.Frames.X / NS.Options.Frames.Scale, NS.Options.Frames.Y / NS.Options.Frames.Scale)
end

--> Format Player Name Only <----------------------------------------
function NS.FormatPlayerNameOnly(playerName)
  local charName = WrapTextInColorCode(gsub(playerName, "-(.*)", ""), select(4, GetClassColor(NS.PlayerDB[playerName].C)))
...
function NS.FormatPlayerNameAndRealm(playerName, GUID)
  ...
  if NS.PlayerDB[playerName] then
    ...
    charNameAndRealm = WrapTextInColorCode(gsub(playerName, "-(.*)", ""), select(4, GetClassColor(NS.PlayerDB[NS.PlayerActiveCache[GUID].Name].C)))
    charNameAndRealm = charNameAndRealm .. " |cffbbbbbb-|r " .. NS.ColorsLUT["realm"]:WrapTextInColorCode(gsub(NS.PlayerActiveCache[GUID].Name, "^(.*-)", ""))
  end
